function coeff=computemultipolesoptimization(rs,js,omega,lmax,rsphere)
mu0=1.25663706*10^-6;
...
ro(:,2)=y(:);
ro(:,3)=z(:);
Ef=EfieldcomputerFMM(ro,rs,js,omega,2);
Ef(:,1)=w(:).*Ef(:,1);
Ef(:,2)=w(:).*Ef(:,2);
Ef(:,3)=w(:).*Ef(:,3);
...
function Ef=EfieldcomputerFMM(ro,rs2,js2,omega,acc)
mu0=1.25663706*10^-6;
eps0=8.85418782*10^-12;
...
[Uz]=lfmm3dpart(acc,nsource(1),rs',1,js(:,3),...
    0,zeros([1 nsource(1)]),zeros([3 nsource(1)]),0,0,nobs,ro',1,1);
Ef(:,3)=Uz.pottarg(1:nobs).';
Ef(:,2)=Uy.pottarg(1:nobs).';
Ef(:,1)=Ux.pottarg(1:nobs).';
end
