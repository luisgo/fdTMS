function coeff=computemultipolesminenergcoil(rs,omega,lmax,rsphere)
mu0=1.25663706*10^-6;
...
nmodes=numel(rs(:,1));
coeff=zeros([2*stack nmodes 3]);
ze=zeros([numel(w(:)) 1]);
for qind=1:numel(rs(:,1))

Ef=EfieldcomputerONE(ro,rs(qind,:),omega);
Ef=w(:).*Ef(:);

for l=1:lmax
    cons=omega*sqrt(2*mu0/(2*l+1)/rsphere);
    cons=1/cons;
    for m=1:1:l
        ind=globind(m,l)+stack;
coeff(ind,qind,1)=sum(Emat{ind}(:).*cat(1,Ef(:),ze,ze),1)*cons;
coeff(ind,qind,2)=sum(Emat{ind}(:).*cat(1,ze,Ef(:),ze),1)*cons;
coeff(ind,qind,3)=sum(Emat{ind}(:).*cat(1,ze,ze,Ef(:)),1)*cons;
    end
    for m=0:1:l
        ind=globind(m,l);
coeff(ind,qind,1)=sum(Emat{ind}(:).*cat(1,Ef(:),ze,ze),1)*cons;
...
function Ef=EfieldcomputerONE(ro,rs,omega)

mu0=-omega*1.25663706*10^-6/(4*pi);
Ef=mu0./sqrt(...
    (ro(:,1)-rs(1)).^2+...
    (ro(:,2)-rs(2)).^2+...
    (ro(:,3)-rs(3)).^2);
