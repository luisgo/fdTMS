function ncoils = runoptiminoga2(depth,energy,Jmax,Srmax,ncoils,harmfile,filename)
lmax=20;
...
clear bine2 Aine2;
Aine2=zeros([2*numel(inde)  ncoils+NA(3)+ncoils]);
inde=1:numel(ASR(:,1));
Aine2(1:numel(inde),1:ncoils)=ASR(inde,1:ncoils)*vmat;
bine2(1:numel(inde))=Srmax;

Aine2(numel(inde)+1:2*numel(inde),1:ncoils)=-ASR(inde,1:ncoils)*vmat;
bine2(numel(inde)+1:2*numel(inde))=Srmax;
Aine2=[];
bine2=[];
%if numel(rangec)~=0
...
ct=1;
thresh=sqrt(2)*50;
bine=thresh*ones([Nang*numel(slack)+nsamp*ncoils 1]);
...
for j=1:Nang
    Aine(j+(i-1)*Nang,1:ncoils)=(A(1,:,i)*sin(th(j))+A(2,:,i)*cos(th(j)))*condi;
        if slack(i)==1
    Aine(j+(i-1)*Nang,ncoils+ct)=-(100-thresh);
