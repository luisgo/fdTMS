...
%harmfile='newharmanytwolayer4mm.mat'
Ncc=24;
dept=125;
filename='optimT3';
filenamesav=strcat(filename,'d',num2str(dept),'ncoils',num2str(Ncc));
designfile=strcat(filenamesav,...
'lmax30.mat');
%%
Jmax=10^6*[4.5524    5.0101    5.7648    6.2035    7.1597];
%gap=10^6*[2.5 2.7 2.8 2.9 3 3.1 3.25];
gap=10^6*5.7648*[1 .9 .8 .75 .7];
ener=[150 150 150 150 150];
% ncoils = runoptiminogaL12layer2(140,ener(:),Jmax(:),gap(:),Ncc,harmfile,filename);
% ncoils = runoptiminoga2(140,ener,Jmax,gap,Ncc,harmfile,filename);
 ncoils = runoptiminoga(dept,ener,Ncc,harmfile,filename);

%%
designfile='optimTd140ncoils1030ncoilstwolayer'
close all
...
filename='optimT3';
filenamesav=strcat(filename,'d',num2str(140),'ncoils',num2str(Ncc));
