function [rs,js,Eout,p,t2p,clen,...
    xcoil,ycoil,zcoil,zzcoil,boo]=coilfieldsp(p,t2p,Att,levs,nx,ny,nz)
...
xc=xcoil{i};yc=ycoil{i};
xcoil{i}=xcoil{i}+.002*nx(xc,yc);
ycoil{i}=ycoil{i}+.002*ny(xc,yc);
zcoil{i}=zcoil{i}+.002*nz(xc,yc);

xcoil{end+1}=xcoil{i}-.002*nx(xc,yc);
ycoil{end+1}=ycoil{i}-.002*ny(xc,yc);
zcoil{end+1}=zcoil{i}-.002*nz(xc,yc);

end
axis equal

clen=0;
boo=(numel(xcoil)>2*numel(levs));
