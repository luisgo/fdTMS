function optim3supports_clusterwinding(iicluster)
if iicluster<=3
harmfile='../halfspheremharm.mat';
Ncc=22;
ct=1;
 dept=[101 131 157]
dept=dept(iicluster);
filename='paperover2optimshalfspherethick';
...
save(strcat('C:\Users\ljg24\Desktop\Coiloptcode\optimize\PAPERhalfspherethickWOUT1002',num2str(dept),'.mat'),'Wi','Si','Ssq','peakJ','peakSR','Escalp','Escalp12','Ndep','Evol','Edecay','Escalp','Ndepsq','Sd','Wd');
peakSR
ct=ct+1;
elseif iicluster<=6
...
filename='paperover2optimsspherethick';
filenamesav=strcat(filename,'d',num2str(dept),'ncoils',num2str(Ncc));
designfile=strcat(filenamesav,'lmax30.mat');
ener=[50 100 150 200 250 300];
peakSR2=10^9*ones(size(ener));
...
[Wi,Si,peakJ,peakSR,Escalp,Escalp12,Ndep,Evol,Edecay,Escalp,Ssq,Ndepsq,Sd,Wd]=anycoildeterminetradeoff_f(harmfile,designfile,dept,Ncc);
save(strcat('C:\Users\ljg24\Desktop\Coiloptcode\optimize\PAPERspherethickWOUT1002',num2str(dept),'.mat'),'Wi','Si','Ssq','peakJ','peakSR','Escalp','Escalp12','Ndep','Evol','Edecay','Escalp','Ndepsq','Sd','Wd');
peakSR
ct=ct+1;
elseif iicluster<=9
%%
harmfile='../squareharm.mat';
Ncc=40;
%for dept=[49 65 79]
ct=1;
dept=[101 131 157]
dept=dept(iicluster-6);
filename='paperover2optimshalfsquarethick';
filenamesav=strcat(filename,'d',num2str(dept),'ncoils',num2str(Ncc));
designfile=strcat(filenamesav,'lmax30.mat');
ener=[50 100 150 200 250 300];
peakSR2=10^9*ones(size(ener));
...
[Wi,Si,peakJ,peakSR,Escalp,Escalp12,Ndep,Evol,Edecay,Escalp,Ssq,Ndepsq,'Sd','Wd']=anycoildeterminetradeoff_f(harmfile,designfile,dept,Ncc);
save(strcat('C:\Users\ljg24\Desktop\Coiloptcode\optimize\PAPERhalfsquarethickWOUT1002',num2str(dept),'.mat'),'Wi','Si','Ssq','peakJ','peakSR','Escalp','Escalp12','Ndep','Evol','Edecay','Escalp','Ndepsq','Sd','Wd');
peakSR
ct=ct+1;
end
