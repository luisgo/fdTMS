function ncoils = runoptiminogaL12layer1(depth,energy,Jmax,ncoils,harmfile,filename)
...
[x, fval, exitflag, output]=runoptimisation(Aloo,jacloo,slackloo, ...
    Avector,Lmat,AL1,vmat,ncoils,stv,env,options,pererr,Nang,condi,Menergy(jj),Jmax(jj),harmfile);
exflag(jj)=exitflag;
%% post-process
x=condi*x;
...
'ncoilstwolayer.mat'),'-v7.3','Jcalc','Jcalc2','Menergy','Lmat','exflag');
end
end

function [x, fval, exitflag, output]=runoptimisation(Aloo,jacloo,slackloo, ...
    Avector,Lmat,AL2,vmat,ncoils,stv,env,options,pererr,Nang,condi,Wmax,Jmax,harmfile)
Jmax=Jmax/condi;
NL1=1;
for loo=1:3
%% adaptive refinement    
...
Jpp=sqrt((Apx(rangec,1:ncoils)*vmat*x(1:ncoils)).^2+...
       ...
       max(Jpp)
boolvar(Jpp(:)>Jmax*.85)=1;
nnz(boolvar)

AL1=generateL1mat_adapt(that1,that2,Apx,Apy,Apz,rangec,ncoils,16,ptt(boolvar==1));
AL1=AL1*vmat;
%% cost function
