...
normi=-mean(abs(Eoutreg(:)))/mean(abs(Eout(:)))/3;
max(Eout(:))
...
normi=X(end)
c=costfnallbig2(X(1:end-1),Eoutreg,pmod2,t2pmod2,Attmod2,normi,Nv,nhhmod)
...
for j=1:numel(Nv);
    stn=sum(Nv(1:j-1));
for i=1:Nv(j)-1
c(ct)=-cdist{j}(levs(stn+i),levs(stn+i+1))+0.0032;
ct=ct+1;
end
end
ceq=[];
