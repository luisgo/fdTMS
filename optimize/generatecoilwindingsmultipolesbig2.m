function [xcoilbig,ycoilbig,zcoilbig,zzcoilbig,...
          ...
          xcoilside,ycoilside,zcoilside,zzcoilside,rsT,jsT]=...
...
[~,~,~,~,Fsym1,Fsym2,Att1,psym,t2psym...
    ]=getcoilwindings(harmfile,designfile,nlayers,desid,6,Ncc);

pause
%[~,~,~,~,~,~,Attc,~,~...
%    ]=getcoilwindings(harmfile,'optimT3d170ncoils24lmax30.mat',nlayers,4,10,24);
%Attc=Attc/max(Attc);
...
trisurf(t2pmod2{i},pmod2{i}(:,1),pmod2{i}(:,2),pmod2{i}(:,3),Attmod2{i})
close all
...
X = fmincon(@(levs)costfnallbig2(levs(1:end-1),Eoutreg,pmod2,t2pmod2,Attmod2,levs(end),Nv,nhhmod),X0,Aine,Bine,[],[],...
    LB,UB,@(levs) mycon(levs(1:end-1),cdist,Nv),options);
normi=X(end)
[c,rsT,jsT]=costfnallbig2rs1js1(X(1:end-1),Eoutreg,pmod2,t2pmod2,Attmod2,normi,Nv,nhhmod);
...
for j=1:numel(Nv);
    stn=sum(Nv(1:j-1));
for i=1:Nv(j)-1
c(ct)=-cdist{j}(levs(stn+i),levs(stn+i+1))+0.0022;
ct=ct+1;
end
end
ceq=[];
